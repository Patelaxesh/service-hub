...
class CustomerPaymentScreen extends StatefulWidget {
  ...
  State<CustomerPaymentScreen> createState() =>
      _CustomerPaymentScreenState(); // Fixed: Public State type
}
...
// Wallets Controller
  ...
  @override
  void dispose() {
// Best practice: Always clean up and dispose all text input controllers
...
// Fixed: Checked async gap structure before handling context updates
    if (!mounted) return;
    ...
    Navigator.pushReplacement(
      context,
      MaterialPageRoute(
        builder: (context) => CustomerMyBookingsScreen(
          serviceName: widget.serviceName,
          finalAmount: widget.totalAmount - _discount,
          bookingDate: "2025-4-15",
          serviceProvider: "Axesh Patel",
          paymentMethod: selectedPaymentMethod ?? "Unknown",
          bookingStatus: "Confirmed",
        ),
  ...
  void _applyCoupon(String code) {
    if (code == "DISCOUNT10") {
      setState(() {
        _discount = widget.totalAmount * 0.1;
...
// Fixed: Replaced deprecated parameters by using RadioGroup structural pattern
              RadioGroup<String>(
                groupValue: selectedPaymentMethod,
                onChanged: (val) => setState(() => selectedPaymentMethod = val),
                child: Column(
                  children: ['Card', 'UPI', 'Net Banking', 'Wallets', 'COD']
...
// Fixed: Checked async context lifecycle validation map
                              if (!mounted) return;
