    ...
    try {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Error fetching bookings: $e', style: const TextStyle(fontWeight: FontWeight.w600)),
  ...
  Future<void> completeBooking(Map<String, dynamic> booking) async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        title: const Text('Mark as Completed?', style: TextStyle(fontWeight: FontWeight.w800, color: textDark)),
        content: const Text('Are you sure you want to change this booking status to completed?'),
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(18)),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(context, false),
            child: const Text('Cancel', style: TextStyle(color: textMuted, fontWeight: FontWeight.w700)),
          ),
          ElevatedButton(
    ...
    if (confirmed == true) {
      try {
        ...
        if (mounted) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: const Text('Booking marked as completed!', style: TextStyle(fontWeight: FontWeight.w600)),
        ...
        if (mounted) {
          Navigator.of(context).pop();
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('Error updating booking: $e'), backgroundColor: primaryColor),
          );
        }
      }
    }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        bottom: PreferredSize(
          ...
          child: Container(
            ...
            child: TabBar(
              ...
              labelStyle: const TextStyle(fontWeight: FontWeight.w800, fontSize: 15, letterSpacing: 0.3),
      ...
      body: TabBarView(
        controller: _tabController,
        children: [
          isLoading ? _buildShimmerList() : _buildOrderList(pendingBookings, true),
          isLoading ? _buildShimmerList() : _buildOrderList(completedBookings, false),
        ],
      ),
    );
  }
                ...
                ...List.generate(4, (i) => Padding(
                  ...
                  child: Row(
                    children: [
                      ...
                      Container(width: i == 2 ? 200 : 110, height: 12, decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(4))),
                    ],
                  ),
                )),
              ],
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            Text(
              isPending ? 'No pending bookings found' : 'No completed bookings found',
              style: const TextStyle(fontSize: 15, color: textMuted, fontWeight: FontWeight.w600),
            ),
          ],
        ),
      );
    ...
    return RefreshIndicator(
      ...
      child: ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          final double finalAmt = rawAmount is num ? rawAmount.toDouble() : (double.tryParse(rawAmount?.toString() ?? '') ?? 0.0);
          ...
          return _buildInfoCard(
            child: Column(
              ...
              children: [
                Row(
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      decoration: BoxDecoration(
                        color: isPending ? accentColor : const Color(0xFFE8F5E9),
                ...
                _buildInfoRow(Icons.person_outline_rounded, booking['customerName'] ?? 'N/A'),
                const SizedBox(height: 6),
                _buildInfoRow(Icons.calendar_today_rounded, _formatBookingDate(booking['bookingDate'])),
                const SizedBox(height: 6),
                _buildInfoRow(Icons.location_on_outlined, booking['address'] ?? 'No address supplied'),
                const SizedBox(height: 6),
                _buildInfoRow(Icons.credit_card_rounded, 'Payment via: ${booking['paymentMethod'] ?? 'Unknown'}'),
                const SizedBox(height: 6),
                _buildInfoRow(
                  Icons.currency_rupee_rounded,
                  NumberFormat.currency(symbol: '₹', decimalDigits: 2).format(finalAmt),
                  isAmount: true,
                ),
                if (isPending) ...[
                  const SizedBox(height: 14),
